...
from Altron.helper_extra.admin_rights import user_can_changeinfo
...
def mpromote(update: Update, context: CallbackContext) -> str:
    ...
    bot.sendMessage(
        chat.id,
        f"Promoting a user in <b>{chat.title}</b>\n\n<b>New Admin:</b> {mention_html(user_member.user.id, user_member.user.first_name)}\n<b>By Admin:</b> {mention_html(user.id, user.first_name)}",
        parse_mode=ParseMode.HTML,
    )
...
def unpin(update: Update, context: CallbackContext):
    ...
    if prev_message and is_group:
        try:
            ...
            msg.reply_text(
                f"» sᴜᴄᴄᴇssғᴜʟʟʏ ᴜɴᴩɪɴɴᴇᴅ <a href='{message_link}'>ᴛʜᴀᴛ ᴍᴇssᴀɢᴇ</a>.",
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True,
            )
        except BadRequest as excp:
...
LOWPROMOTE_HANDLER = DisableAbleCommandHandler("spromote", spromote)
...
𝗧𝗵𝗲 𝗙𝗼𝗹𝗹𝗼𝘄𝗶𝗻𝗴 𝗖𝗼𝗺𝗺𝗮𝗻𝗱𝘀 𝗮𝗿𝗲 𝗔𝗱𝗺𝗶𝗻𝘀 𝗼𝗻𝗹𝘆:
  ...
  ➲ /spromote: ᴘʀᴏᴍᴏᴛᴇꜱ ᴛʜᴇ ᴜꜱᴇʀ ʀᴇᴘʟɪᴇᴅ ᴛᴏ ᴡɪᴛʜ ʟᴏᴡ ʀɪɢʜᴛꜱ
...
command_list = ['admins', 'pinned', 'pin', 'unpin', 'invitelink', 'promote', 'fullpromote', 'mpromote', 'spromote', 'demote', 'title', 'admincache', 'setgtitle', 'setgpic', 'setdesc', 'setsticker']
