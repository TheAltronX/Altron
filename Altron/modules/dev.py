...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        update.effective_message.reply_text(f"Current state: {Altron.ALLOW_CHATS}")
        return
    if args[0].lower() in ["off", "no"]:
        Altron.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        Altron.ALLOW_CHATS = False
    else:
        update.effective_message.reply_text("Format: /lockdown Yes/No or On/Off")
        return
