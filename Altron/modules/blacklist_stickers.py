...
import Altron.modules.sql.blsticker_sql as sql
from Altron import LOGGER, dispatcher
...
from Altron.modules.warns import warn
...
def add_blackliststicker(update: Update, context: CallbackContext):
    ...
    if len(words) > 1:
        ...
        if len(to_blacklist) == 1:
            send_message(
                update.effective_message,
                "✅ Added to blacklist stickers in <b>{}</b>!\n\nSticker: <code>{}</code>".format(
...
𝗡𝗼𝘁𝗲: <sticker link> ᴄᴀɴ ʙᴇ `https://t.me/addstickers/<sticker>` ᴏʀ ᴊᴜꜱᴛ `<sticker>` ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴛʜᴇ ꜱᴛɪᴄᴋᴇʀ ᴍᴇꜱꜱᴀɢᴇ
𝗡𝗼𝘁𝗲: ʙʟᴀᴄᴋʟɪꜱᴛ ꜱᴛɪᴄᴋᴇʀꜱ ᴅᴏ ɴᴏᴛ ᴀꜰꜰᴇᴄᴛ ᴛʜᴇ ɢʀᴏᴜᴘ ᴀᴅᴍɪɴ
"""
...
command_list = ['blsticker', 'addblsticker', 'unblsticker', 'rmblsticker', 'blstickermode']
